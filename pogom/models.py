...
from .utils import get_pokemon_name, get_pokemon_rarity, get_pokemon_types, get_args, \
    ...
    get_move_energy, get_move_type, get_gmaps_altitude, randomize_altitude
...
class Pokemon(BaseModel):
    ...
    def get_spawnpoints(cls, swLat, swLng, neLat, neLng, timestamp=0, oSwLat=None, oSwLng=None, oNeLat=None, oNeLng=None):
        query = Pokemon.select(Pokemon.latitude, Pokemon.longitude, Pokemon.spawnpoint_id, (date_secs(Pokemon.disappear_time)).alias('time'), fn.Count(Pokemon.spawnpoint_id).alias('count')).where(Pokemon.spawnpoint_id.is_null(False))
...
class LocationAltitude(BaseModel):
    ...
    # return altitude from the db or try to fetch from gmaps api, otherwise, default altitude
    @classmethod
...
def parse_map(args, map_dict, step_location, db_update_queue, wh_update_queue, api, now_date):
    ...
    if len(wild_pokemon):
        ...
        for p in wild_pokemon:
            ...
            construct_pokemon_dict(pokemons, p, encounter_result, disappear_time, lure_info=None)
                    ...
                    if lure_info is not None and config['parse_pokemon']:
                        ...
                        if (args.encounter and (lure_info['active_pokemon_id'] in args.encounter_whitelist or
                            ...
                            encounter_result = api.disk_encounter(encounter_id=lure_info['encounter_id'],
                                                                  ...
                                                                  player_latitude=step_location[0],
                                                                  player_longitude=step_location[1])
                        construct_pokemon_dict(pokemons, f, encounter_result, disappear_time, lure_info)
...
def create_tables(db):
    ...
    db.create_tables([Pokemon, Pokestop, Gym, ScannedLocation, GymDetails, GymMember, GymPokemon,
                      Trainer, MainWorker, WorkerStatus, SpawnPoint, ScanSpawnPoint, SpawnpointDetectionData, LocationAltitude], safe=True)
    db.close()
...
def drop_tables(db):
    ...
    db.drop_tables([Pokemon, Pokestop, Gym, ScannedLocation, Versions, GymDetails, GymMember, GymPokemon,
                    Trainer, MainWorker, WorkerStatus, SpawnPoint, ScanSpawnPoint, SpawnpointDetectionData, LocationAltitude, Versions], safe=True)
    db.close()
